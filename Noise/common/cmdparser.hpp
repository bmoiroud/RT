...
#define    TEST_3D_NOISE        1
#define    TEST_CLOUD           2
#define    TEST_2D_NOISE        4

int parseTest (const string& test_name);


class CmdParserTest : public CmdParserDeviceType
{
public:

    CmdParserTest (int argc, const char** argv);

    CmdOption<string> run;
    CmdOption<int> seed;
...
class CmdParserCommon : public CmdParserTest
{
public:
...
public:
    CmdOptionErrors (CmdParser& cmd_parser) :
        ...
        (
            ...
            "errors",
            "<integer>",
            "Number of validation errors (if any) to print.",
            10
        )
    {}
};
