...
/*   Updated: 2017/12/16 19:10:05 by bmoiroud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	ft_ref_color(__global t_rt *rt, t_ray *ray, const double ref, __constant double *rand)
{
	...
	if (ray->id != id && ray->id < rt->nb_obj && rt->objects[ray->id].transp && rt->effects)
...
static t_color	ft_reflection(__global t_rt *rt, t_ray *ray, __constant double *rand)
{
	...
	while ((((rt->objects[ray->id].reflect && ray->bounces) || \
		rt->objects[ray->id].refract || rt->objects[ray->id].transp)) && reflection > 1.0)
	{
		if (rt->objects[ray->id].reflect)
		{
		...
		if (rt->objects[ray->id].refract)
		{
			c_ref = ft_refract_color(rt, 1.0, rt->objects[ray->id].refract, ray);
			light = -0.075;
		}
